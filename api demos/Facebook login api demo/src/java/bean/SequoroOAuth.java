...
public class SequoroOAuth {
    ...
    public SequoroOAuth() throws UnsupportedEncodingException, IOException {
        ...
        clientId = "Uo2kLu3y/G1j/VoXL7R7XlMSC0U=";
        clientSecretKey = "ODY4MDg5NjcwdGVzdEBnbWFpbC5jb21GYWNlYm9va2g=";
        redirectUrl = "http://localhost:8080/facebook/login_check.jsp";
        authUrl = "http://localhost:8080/site/api/accesscode_api.jsp?clientid=" + this.clientId + "&clientsecretkey=" + this.clientSecretKey + "&requestcode=";
        initializeUrl = "http://localhost:8080/site/api/request_intialize.jsp?clientid=" + this.clientId + "&clientsecretkey=" + this.clientSecretKey;
        state = initialize_request().trim();
        time = new Timestamp(new Date().getTime()).toString();
        requestUrl = "http://localhost:8080/site/api/login_api.jsp?clientid=" + clientId + "&redirecturl=" + redirectUrl + "&time=" + time + "&state=" + state;
