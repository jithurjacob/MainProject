...
<%! String client_id,original_client_id,state,redirect_url,original_redirect_url,nonce,clientsecret;Timestamp time;Boolean valid=true; %>
...
//    if((((((ts_now.getTime()-time.getTime())/1000)/60)/1000)/60)>5)
//        valid=false;
...
<jsp:setProperty name="api_obj" property="clientid" value="<%=client_id%>" />
<%
try{
    System.out.println("websiteclientid : "+website_obj.getClient_id());
    System.out.println(website_obj.getSite_details(website_obj.getClient_id()));
...
System.out.println("Excn in getting : "+ex.toString());
}
%>
<%!
 boolean verifyNonce(String baseurl) {
     try{
     System.out.println("Baseurl : "+baseurl);
     String requestUrl=baseurl+"api/login_api.jsp?clientid=" + this.original_client_id+ "&redirecturl=" + this.original_redirect_url + "&time=" + time+"&state="+this.state;
     ...
     System.out.println("Excn in vernonce : "+ex.toString());
     }
     return false;
}
%>
    ...
    <body class="bg api">
        <section id="demo" style="padding: 10px">
            <div class="container">
                ...
                <%
                boolean validnc=verifyNonce(con_obj.baseurl);
                boolean validre=website_obj.isValidRequest();
                boolean validst=api_obj.isValidState(state);
                
                System.out.print(validre+""+valid+""+validnc+""+validst);
                   try{
                if ( validre && valid && validnc && validst ) {
                ...
                <%
                   ...
                   System.out.println("Excni in ifelse : "+ex.toString());
                   }
                %>
                    ...
                    <div class=" text-center">
                        <button id="final" type="button" class="example4demo btn btn-default hide" result="invalid" onclick="CloseMySelf(this);">final</button>
                    </div>
            ...
            //}   
                            ...
                            function CloseMySelf(sender) {
                                ...
                                try {
                                  ...
                                  // alert(5);
				window.opener.postMessage(sender.getAttribute("result"),"*");
                                }
                                catch (err) {
                        ...
                        success: function (result) {
                            ...
                            if (res == "notok") {
                                ...
                                if(res=="ok"){
                                    ...
                                    $("#final").attr('result',JSON.stringify(result));
                                    //alert($("#final").data('result'));
                                    $("#final").click();
                                }else
                                $('#result_div').html(res);
                            }
