...
try{  
Connection con=ConnectionProvider.getCon();
PreparedStatement ps1=con.prepareStatement("select count(*) as count , verified from user_details where email='"+this.getEmail()+"' and  pasw='"+this.getPasw()+"'");
...
if(count<=0){
    if(this.getName()==null)
        this.setName("demo");
PreparedStatement ps=con.prepareStatement("insert into user_details (name,pasw,email) values('"+this.getName()+"','"+this.getPasw()+"','"+this.getEmail()+"');");  
