		...
		return $('.smint a').each( function() {
			...
			var stickyMenu = function(){
				...
				if (scrollTop > stickyTop) { 
					$('.smint').css({ 'position': 'fixed', 'top':0,'width':'100%','background-color': '#202020'}).addClass('fxd');
					//$('.smint').css({ }).addClass('fxd');
				} else {
						$('.smint').css({ 'position': 'inherit','background-color': 'rgba(26, 26, 26, 0.52)' }).removeClass('fxd');
						//$('.smint').css({ }).removeClass('fxd');
						$('.smint').removeProp('width'); 
