window.addEventListener("message", function(e) {
    ...
    var result=e.data;
      ...
      window.location.reload(true);
           //   window.location=window.location.href+"?attempt="+$.now();
    }else {
        result=JSON.parse(result);
        if(result.status=="ok"){
            window.location="<%=sequoroOAuth_reg.getRedirectUrl()%>?provider=sequoro&requestcode="+result.requestcode;
        }else{
...
function sequoro_reg() {

    var a = $('#sequoro_btn').popupWindow({
        windowURL: '<%=sequoroOAuth_reg.getRequestUrl()%>',
        windowName: 'swip',
        centerScreen: 1,
        scrollbars: 1
